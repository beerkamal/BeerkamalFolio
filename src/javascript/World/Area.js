...
export default class Area extends EventEmitter {
  ...
  setFloorBorder() {
    ...
    this.floorBorder.geometry = new AreaFloorBorderGeometry(
      this.halfExtents.x * 2,
      this.halfExtents.y * 2,
      0.25
    );
    this.floorBorder.material = new AreaFloorBordereMaterial();
    this.floorBorder.material.uniforms.uColor.value = new THREE.Color(0xf8f8f8);
  ...
  setFence() {
    ...
    // Geometry
    this.fence.geometry = new AreaFenceGeometry(
      this.halfExtents.x * 2,
      this.halfExtents.y * 2,
      this.fence.depth
    );
  ...
  setKey() {
    ...
    this.key.enter.geometry = new THREE.PlaneGeometry(
      this.key.enter.size,
      this.key.enter.size / 4,
      1,
      1
    );
    ...
    this.key.enter.mesh = new THREE.Mesh(
      this.key.enter.geometry,
      this.key.enter.material
    );
    ...
    this.key.icon.geometry = new THREE.PlaneGeometry(
      this.key.icon.size,
      this.key.icon.size,
      1,
      1
    ...
    this.key.icon.mesh = new THREE.Mesh(
      this.key.icon.geometry,
      this.key.icon.material
    );
  ...
  interact(_showKey = true) {
    ...
    gsap.to(this.fence.mesh.position, {
      ...
      onComplete: () => {
        ...
        gsap.fromTo(
          this.floorBorder.material.uniforms.uAlpha,
          { value: 1 },
          { value: 0.5, duration: 1.5 }
        );
        gsap.fromTo(
          this.fence.material.uniforms.uBorderAlpha,
          { value: 1 },
          { value: 0.5, duration: 1.5 }
        );
      },
  ...
  setInteractions() {
    ...
    this.time.on("tick", () => {
      if (this.testCar) {
        const isIn =
          ...
          Math.abs(this.car.position.y - this.position.y) <
    ...
    window.addEventListener("keydown", (_event) => {
      if (
        (_event.key === "f" || _event.key === "e" || _event.key === "Enter") &&
        this.isIn
      ) {
        this.interact();
      }
